...
Durante il training stampa progresso per epoca (`qinn_progress_every`) e salva `loss.pdf` nella cartella output.

Per il wrapper condor puoi forzare il routing:
- `FASTCALO_QINN_STANDALONE=1` forza `train_qinn.py` / `evaluate_qinn.py`
- `FASTCALO_QINN_LEGACY_GAN=1` forza `train.py` / `evaluate.py`
